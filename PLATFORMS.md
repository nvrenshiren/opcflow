...
opcflow 是 vibecode 平台无关的执行引擎:核心(SQLite 数据层 / gates / 信任协议 / DAG /
...
在项目根直接跑 `init`,不带参数即进入**交互引导**(选语言 / 平台多选 / 端 / 模型,模型清单从 models.dev 拉取):

```bash
npx -y @dawipong/opcflow init
```
...
- **OpenCode**:模型串是 `provider/model` 格式(见 [models.dev](https://models.dev));API key
  ...
  opcflow 生成一个薄壳插件转发到 hook 脚本。
- **各平台文档更新快**:字段/路径以核实当日为准,接入前对照目标平台当前版本再校一次。

## 模型来源

交互式 `init` 从 [models.dev](https://models.dev) 的 `api.json` 拉取「支持 tool_call」的模型清单,
按 provider 分组两级选择(先选供应商再选模型);也可用 `--model` 直接指定或用各平台默认。
