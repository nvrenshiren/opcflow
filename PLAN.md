...
### workbench.config.json（项目层，本项目初始值）

```jsonc
{
  "endpoints": ["service", "admin", "weapp", "app"],
  "docs": {
    "prd": "docs/prd", "architecture": "docs/architecture",
    "design": "docs/design", "acceptance": "docs/acceptance"
  },
  "codeRoots": {
    "service": "service/src/modules/{client}/{module}",
    "admin":   ["admin/src/pages/home/{module}", "admin/src/components/form/{module}"],
    "weapp":   "weapp/src/pages/{module}",
    "app":     "app/lib/features/{module}"
  },
  "machineChecks": {              // M5 接线时以各 package.json scripts 核实为准
    "service": ["pnpm --filter service exec tsc --noEmit"],
    "admin":   ["pnpm --filter admin exec tsc --noEmit"],
    "weapp":   ["pnpm --filter bangxiaoer exec tsc --noEmit"],
    "app":     ["flutter analyze"]
  },
  "protocolLints": [              // 协议 lint 集(M5)
    { "name": "no-enum-literal", "grep": "z\\.enum\\(\\[\"", "paths": ["service/src"] },
    { "name": "no-svg-in-weapp", "grep": "<svg",             "paths": ["weapp/src"] },
    { "name": "no-page-size",    "grep": "pageSize",          "paths": ["service/src/modules"] }
  ],
  "moduleMapping": {              // fine→coarse 归并(沿用 architect memory 既有映射)
    "landType": "land", "community": "land", "appointment": "trade",
    "contractTemplate": "contract", "newsCategory": "news", "adSpace": "ad"
  },
...
### M8 进化机制（约 3–4 天，独立成章——原塞在 M7 的 1.5 天里被严重低估）
...
| 数据单点 | approvals.json 入 git ✅;events/feedback 定期导出 jsonl 入 git（M8 顺手） |
