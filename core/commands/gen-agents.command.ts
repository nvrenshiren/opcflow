...
export function genAgents(ctx: Ctx, templatesDir?: string): GenAgentsResult {
  ...
  const outDir = join(ctx.root, ".claude", "agents")
  ...
  for (const name of readdirSync(dir)) {
    ...
    if (!pipeline.has(agentName)) continue
    let content = readFileSync(join(dir, name), "utf-8")
    const perFile = { ...tokens, AGENT_MEMORY_DIR: `.claude/agent-memory/${agentName}/` }
    for (const [token, value] of Object.entries(perFile)) {
      content = content.replaceAll(`{{${token}}}`, value)
    }
    const leftover = content.match(/\{\{[A-Z_]+\}\}/)
    if (leftover) throw new Error(`模板 ${name} 存在未解析 token: ${leftover[0]}`)
    writeFileSync(join(outDir, name), content)
    written.push(`.claude/agents/${name}`)
  }
  return { written }
}
