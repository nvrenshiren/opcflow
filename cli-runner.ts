...
async function promptInit(): Promise<InitAnswers> {
  ...
  const platforms = await checkbox<string>({
    message: zh ? "平台(空格键多选,回车确认)" : "Platforms (space to toggle, enter to confirm)",
    choices: PLATFORM_IDS.map(p => ({ name: p, value: p, checked: p === "claude" })),
    required: true
  })
...
export async function runCommand(ctx: Ctx, command: string, a: Record<string, any>): Promise<void> {
  switch (command) {
    ...
    case "scan": {
      ...
      console.log(chalk.green(`✅ 扫描完成:新登记 ${s.registered},内容刷新 ${s.refreshed},重挂坐标 ${s.remapped},自动跟随移动 ${s.moved},新增边 ${s.edges},清理残留边 ${s.edgesPruned},排除元产物 ${s.skipped.length}`))
