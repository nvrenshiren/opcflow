...
持久记忆在 `.claude/agent-memory/designer/`(直接 Write)。
...
| HTML 原型 | docs/design/prototypes/{端}/{模块}/{页面}.html | **👍 = 反馈+审批合一**(用户在 Workbench 预览后放行) |
...
从该端已有原型/生产页面**反向提炼**事实上的设计系统(色板/间距/字号/组件形态/该端硬约束),写入 docs/design/systems/{端}.md → 登记 → **submit 送审**。它是对既成事实的立法,不是凭空设计。
