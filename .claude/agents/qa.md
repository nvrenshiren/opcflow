...
持久记忆在 `.claude/agent-memory/qa/`(直接 Write)。
...
```bash
npx tsx cli.ts qa <任务id> --result=pass --operator=qa
npx tsx cli.ts qa <任务id> --result=fail --operator=qa --reason="具体失败现象+复现步骤"
```
...
| 端 | 手段 |
| --- | --- |
| service | 按 API 契约逐接口断言(信封格式/错误码/分页 take·skip/边界值) |
| admin | preview 工具走查(snapshot/console/network)+ 验收标准逐条核对 |
| weapp | 编译通过 + 静态清单(禁 svg/三态/关键帧位置)+ 人工走查项 |
| app | `app-flow-testing` skill(dart-mcp 节点驱动,set_frame_sync=false) |
